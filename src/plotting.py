...
def set_projection_params(hemi, view, cmap='cold_hot', inflated=False, threshold=1e-15, colorbar=False, symmetric_cbar=False, template=None, figure=None, ax=None, vmax=None):
...
def create_grid(nb_rows, nb_columns, zooming_factor=30, row_size_factor=4, overlapping=6, column_size_factor=5, wspace=-0.15, hspace=-0.15, top=1., bottom=0., left=0., right=1):
  ...
  axes = [[figure.add_subplot(gs[int(max(0, i*(zooming_factor-6))):int((i+1)*zooming_factor)-i*6, int(max(0, j*zooming_factor-overlapping)):int((j+1)*zooming_factor+overlapping)], projection= '3d') for j in range(1, 1+nb_columns)] for i in range(nb_rows)]
...
def pretty_plot(
  ...
  figure, axes = create_grid(nb_rows=len(names), nb_columns=4, row_size_factor=6, overlapping=6, column_size_factor=12)
